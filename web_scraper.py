...
# def recupera_dettagli_evento(url_evento):
...
#     try:
#         response = requests.get(url_evento, headers=headers)  # Usa le stesse intestazioni di prima
#         response.raise_for_status()
#         soup = BeautifulSoup(response.content, "html.parser")
#         dettagli_div = soup.find("div", class_="eve-details")
#         if dettagli_div:
#             return str(dettagli_div)  # Restituisci il contenuto come stringa
#         else:
#             return None  # Restituisci None se il div non viene trovato
#     except requests.exceptions.RequestException as e:
#         print(f"Errore durante la richiesta della pagina evento: {e}")
#         return None
